...
func (h *DPDKHandle) ReadPacketData() ([]byte, gopacket131_dpdk.CaptureInfo, error) {
	h.mu.Lock()
	defer h.mu.Unlock()
	...
	if h.currentIdx < 0 || h.currentIdx >= len(h.mbufs) {
		return nil, gopacket131_dpdk.CaptureInfo{}, fmt.Errorf("currentIdx out of bounds: %d", h.currentIdx)
	}
