...
// serially instead of in parallel, and uses gopacket131_dpdk.Packet instead of
// gopacket131_dpdk.DecodingLayerParser for packet processing.  We also make use of very
...
type scanner struct {
	...
	buf  gopacket131_dpdk.SerializeBuffer
}
...
func (s *scanner) getHwAddr() (net.HardwareAddr, error) {
	...
	for {
		...
		packet := gopacket131_dpdk.NewPacket(data, layers.LayerTypeEthernet, gopacket131_dpdk.NoCopy)
...
func (s *scanner) scan() error {
	...
	ipFlow := gopacket131_dpdk.NewFlow(layers.EndpointIPv4, s.dst, s.src)
	start := time.Now()
	for {
...
func (s *scanner) send(l ...gopacket131_dpdk.SerializableLayer) error {
	if err := gopacket131_dpdk.SerializeLayers(s.buf, s.opts, l...); err != nil {
		return err
	}
