...
// gopacket131_dpdk and its subpackages.  This example shows, among other things:
...
func readARP(handle *pcap.Handle, iface *net.Interface, stop chan struct{}) {
	src := gopacket131_dpdk.NewPacketSource(handle, layers.LayerTypeEthernet)
	in := src.Packets()
	for {
		var packet gopacket131_dpdk.Packet
		select {
		case <-stop:
			return
		case packet = <-in:
...
func writeARP(handle *pcap.Handle, iface *net.Interface, addr *net.IPNet) error {
	...
	for _, ip := range ips(addr) {
		...
		gopacket131_dpdk.SerializeLayers(buf, opts, &eth, &arp)
