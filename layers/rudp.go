...
// LayerType returns gopacket131_dpdk.LayerTypeRUDP.
func (r *RUDP) LayerType() gopacket131_dpdk.LayerType { return LayerTypeRUDP }

func decodeRUDP(data []byte, p gopacket131_dpdk.PacketBuilder) error {
	r := &RUDP{
		SYN:          data[0]&0x80 != 0,
		ACK:          data[0]&0x40 != 0,
		EACK:         data[0]&0x20 != 0,
...
func (r *RUDP) TransportFlow() gopacket131_dpdk.Flow {
	return gopacket131_dpdk.NewFlow(EndpointRUDPPort, []byte{byte(r.SrcPort)}, []byte{byte(r.DstPort)})
}
