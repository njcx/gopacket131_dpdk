...
class Packet(object):
  ...
  def Test(self, name, link_type):
    ...
    yield '\tp := gopacket131_dpdk.NewPacket(testPacket%s, LinkType%s, gopacket131_dpdk.Default)' % (name, link_type)
    yield '\tif p.ErrorLayer() != nil {'
    yield '\t\tt.Error("Failed to decode packet:", p.ErrorLayer().Error())'
    yield '\t}'
    yield '\tcheckLayers(p, []gopacket131_dpdk.LayerType{LayerType%s, FILL_ME_IN_WITH_ACTUAL_LAYERS}, t)' % link_type
    yield '}'
    yield 'func BenchmarkDecodePacket%s(b *testing.B) {' % name
    yield '\tfor i := 0; i < b.N; i++ {'
    yield '\t\tgopacket131_dpdk.NewPacket(testPacket%s, LinkType%s, gopacket131_dpdk.NoCopy)' % (name, link_type)
    yield '\t}'
    yield '}'
...
def main():
  class CustomHelpFormatter(argparse.ArgumentDefaultsHelpFormatter):
    def _format_usage(self, usage, actions, groups, prefix=None):
      header =('TestCreator creates gopacket131_dpdk tests using a pcap file.\n\n'
