...
func testCheckFSM(t *testing.T, options TCPSimpleFSMOptions, s []testCheckFSMSequence) {
	...
	for i, test := range s {
		...
		if res != test.expected {
			t.Fatalf("#%d: packet rejected (%v): got %v, expected %v. State:%s", i, gopacket131_dpdk.LayerDump(&test.tcp), res, test.expected, fsm.String())
		}
	}
}
...
func testCheckOptions(t *testing.T, title string, s []testCheckOptionsSequence) {
	...
	for i, test := range s {
		...
		res := err == nil
		if res != test.expected {
			t.Fatalf("'%v' #%d: packet rejected (%v): got %v, expected %v.", title, i, gopacket131_dpdk.LayerDump(&test.tcp), res, test.expected)
		}
	}
}
